...
for (const [group, terms] of Object.entries(RULES.types)) {
    if (includesAny(text, terms)) {
        console.log(`  RULES.types.${group} match`);
    }
}
for (const [group, terms] of Object.entries(RULES.evidence)) {
    if (includesAny(text, terms)) {
        console.log(`  RULES.evidence.${group} match`);
    }
}
if (includesAny(text, RULES.useful)) console.log(`  RULES.useful match`);
if (includesAny(text, RULES.noise)) console.log(`  RULES.noise match`);
